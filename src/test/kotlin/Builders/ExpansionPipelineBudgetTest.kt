...
/**
 * Pins the contract that every pipe in ExpansionPipeline carries a
 ...
 *
 * Mirrors PlusWriterPipelineBudgetTest and DialogueConnectorBudgetTest.
 * Does NOT call init() or hit the network. Pure reflection on the
 * built pipeline's pipe list. A fake API key is wired into
 * [GenericOpenAIEnv] in [BeforeAll] so that buildExpansionPipeline()
 * can construct its reasoning sub-pipes without throwing.
 */
class ExpansionPipelineBudgetTest
{
    companion object
    {
        @JvmStatic
        @BeforeAll
        fun setUpApiKey() {
            // buildExpansionPipeline() instantiates GenericOpenAIPipe via
            // its reasoning sub-pipes (explicitCotBuilder + authorBuilder +
            // various Env.* prompts); those sub-pipes' init() validates the
    ...
    fun everyPipeHasTokenBudgetSettings() {
        ...
        assert(pipes.isNotEmpty()) { "buildExpansionPipeline() returned no pipes" }
